...
(defun cannot-go (loc dir)
  (not (member dir (mapcar #'car (location-exits loc)))))
    ...
    (cond ((and (or (and (eq loc 'west-bridge) (eq dir 'e))
          ...
          ((and (eq loc 'banqueting-hall)
                ...
                (not (or (table-get 'wearing-uniform)
                         (table-get 'wearing-boots))))
           "Grargs will see you!")
          ((and (member loc '(campfire ploughed-field sentry-post guard-room))
                (not (table-get 'wearing-uniform))
                (not (table-get 'wearing-boots)))
          ...
          ((and (member loc '(row-of-casks wine-cellar))
...
(defun examine-handler (loc item default)
  (cond ((not item)
        ...
        ((and (eq loc 'sluice-gates)
              (member item '(gates sluice-gates)))
         "There is a handle.")
        ((eq item 'phial)
        ...
        ((and (eq loc 'well-bottom)
              (member item '(walls well-bottom)))
         "There are loose bricks.")
        ((eq loc 'mosaic-hall)
         "Very interesting!")
        ((member item '(fountain statue pinnacle))
         "Interesting!")
...
(defun give-handler (loc item default)
  (cond ((and (eq loc 'valley) (eq item 'brooch))
        ...
        ((and (eq item 'coin)
              (= (table-get 'money) 0))
         "You have run out!")
        ((member loc '(west-bridge east-bridge))
        ...
        ((and (eq loc 'glass-gates)
         ...
         "He is distracted.")
        ((and (member loc '(glass-gates barren-countryside)))
         (set-item-location item nil)
         "He eats it!")
        ((and (eq loc 'wine-cellar)
              (member item '(apples bread)))
         (set-item-location item nil)
  ...
  (let ((loc *current-location*))
    (cond ((eq item 'magic-words)
          ...
          ((or (not (eq loc 'silver-chamber))
               (not (member item '(awake guide protect lead help)))
...
(defun use-handler (loc item default)
  (cond ((and (eq loc 'lake-edge)
         ...
         "OK.")
        ((member item '(coins apples bone bread apple coin))
...
(defun fill-handler (loc item default)
  (cond ((and (member loc '(rough-water lake-middle))
              (member item '(bucket jug)))
         (table-set 'quest-failed t)
         "You capzised!")
...
(defun dive-handler (loc item default)
  (cond ((member loc '(misty-pool lake-middle))
         (table-set 'quest-failed t)
         "You have drowned.")
        (t default)))
...
(defun ring-handler (loc item default)
  (cond ((and (eq loc 'silver-bell)
         ...
         (let ((times (parse-integer (input "How many times? "))))
           (cond ((= times (table-get 'rings-needed))
  ...
  (flet ((hash-to-alist (hash)
    ...
    (handler-case
        (with-open-file (*standard-output* filename :direction :output
          ...
          (writeln *tunnel-forward*)
          (writeln *tunnel-backward*)
          t)
      (error (c)
...
(defun game-load (filename)
  (flet ((hash-from-alist (hash alist)
           (clrhash hash)
           (mapc (lambda (keyval)
    ...
    (handler-case
        (with-open-file (*standard-input* filename :direction :input)
          ...
          (setf *tunnel-forward* (read))
          (setf *tunnel-backward* (read))
          t)
      (error (c)
        (format t "An error occurred: ~a.~%" c)
        nil))))
